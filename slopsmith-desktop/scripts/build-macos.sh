...
bundle_binaries_impl() {
    ...
    # vgmstream-cli — use the Homebrew-installed binary. The upstream
    # GitHub release asset `vgmstream-mac-cli.zip` was removed/renamed
    # in r2117 (the `releases/latest/download/...` URL 404s), so the
    # old download-from-release step no longer works. `brew install
    # vgmstream` provides a working vgmstream-cli on PATH instead.
    local vgm_bin
    vgm_bin="$(command -v vgmstream-cli || true)"
    if [[ -z "$vgm_bin" ]]; then
        echo "Error: vgmstream-cli not found on PATH. Install it with \`brew install vgmstream\`." >&2
        exit 1
    fi
    ...
    echo "  ✓ vgmstream-cli (from $vgm_bin)"
